...
const recommendedFood = weight => {
  return (weight ** 0.75 * 28).toFixed(2);
};
...
console.log(recommendedFood(sarahsDog.weight));
console.log(sarahsDog.curFood);

if (recommendedFood(sarahsDog.weight) * 1.1 < sarahsDog.curFood) {
  console.log("Sarah's dog eating too much");
} else if (sarahsDog.curFood < recommendedFood(sarahsDog.weight) * 1.1)
  console.log('Sarahs dog eating outside limit');
...
console.log(
  `${ownersEatTooMuch.map(
    (owner, index, arr) => owner + (index !== arr.length - 1 ? ' and ' : "'s")
  )} dog eating too much`
);

console.log(
  `${ownersEatTooLittle.map(
    (owner, index, arr) => owner + (index !== arr.length - 1 ? ' and ' : "'s")
...
//console.log(dogs.map(dog => dog.recommendedPortion === dog.curFood));
...
const okEaterDogs = dogs.filter(
  dog =>
    dog.curFood > dog.recommendedPortion * 0.9 &&
    dog.curFood < dog.recommendedPortion * 1.1
);
console.log(okEaterDogs);

//Solution 2:
console.log(dogs.filter(checkEatingOkay))

// 8. Create a shallow copy of the dogs array and sort it by recommended food portion in an ascending order (keep in mind that the portions are inside the array's objects)
const sortedDogs = dogs.slice().sort(
